...
**CRITICAL: Always run `cargo fmt` before committing or pushing. CI fails on unformatted code.**
...
- **docs.rs** -- `subcontext docs <path>`: dump bundled documentation and sample skills to a directory
- **mcp.rs** -- `subcontext mcp`: JSON-RPC 2.0 MCP server over stdio (`subcontext_status`, `subcontext_deadlines` tools)
- **task.rs** -- Task and board management: add/update/done/fail tasks, boards, deadlines, path resolution
- **global.rs** -- Global (system) and user subcontext: install, child registration, task scope, tree view
- **project.rs** -- Project config (UUID, kind, version) on the config branch
- **namespace.rs** -- Namespace dictionary for human-readable name-to-UUID mapping
- **submodule.rs** -- Overlay submodule management (add, update, remove)
...
**Branch scheme**: `config` (shared), `overlay/<sanitized-branch>` (per-host-branch), `state` (tasks database). Branch sanitization: `/` -> `-`, strip leading dots.

**Documentation**: Bundled docs live in `docs/` and are embedded at compile time via `include_str!()` in `src/docs.rs`. The `subcontext docs <path>` command dumps them to a directory so Claude Code (or users) can read setup guides and sample skills. Sample skills in `docs/skills/` demonstrate the Claude Code skill format and can be adapted per-repository.
